...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for location, prob in self.beliefs.items():
            Distance = util.manhattanDistance(location, pacmanPosition)
            allPossible[location] = prob * emissionModel[Distance]
            sumProb += allPossible[location]
        if sumProb == 0:
            self.initializeUniformly(gameState)
        else:
            allPossible.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        particlePos = []
        for i in xrange(self.numParticles):
            particlePos.append(util.sample(self.beliefs))

        for pos in self.legalPositions:
            self.beliefs[pos] = 0
        for pos in particlePos:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in xrange(self.numParticles):
            ...
            self.particles.append(tuple(positions))
            #    util.raiseNotDefined()
        ...
        for oldParticle in self.particles:
            ...
            for i in xrange(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                             self.ghostAgents[i])
                ghostPositions.append(util.sample(newPosDist))
            newParticle = ghostPositions
    ...
    def observeState(self, gameState):
        ...
        for pos in xrange(self.numGhosts):
            if noisyDistances[pos] != None:
               for j in beliefDis:
                   position = j[pos]
                   trueDistance = util.manhattanDistance(position, pacmanPosition);
                   beliefDis[j] *= emissionModels[pos][trueDistance]
        ...
        for pos in range(self.numGhosts):
            if noisyDistances[pos] == None:
                ...
                for j in beliefDis:
                   new_dis[self.getParticleWithGhostInJail(j, pos)] += beliefDis[j]
                beliefDis = new_dis

        if beliefDis.totalCount() == 0:
           self.initializeParticles()
        else:
           for pos in xrange(self.numParticles):
               self.particles[pos] = util.sample(beliefDis)
