...
class GreedyBustersAgent(BustersAgent):
    ...
    """
        ...
        for i in xrange(len(livingGhostPositionDistributions)):
            ...
            #        maxProbPos = pos
            #        maxProb = prob
            maxProbPos = max(beliefs.iteritems(), key=operator.itemgetter(1))[0]


            Pos_Expt.append(maxProbPos)
       ...
       # for pos in Pos_Expt:
        ...
        for action in legal:
            ...
            if distance < minDistance:
                minDistance = distance
                bestAction = action
        # bestActions = [action for action in legal
        #                if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestPos) == minDistance]
